    ...
    <v-flex>
        ...
        <v-dialog v-model="comprobanteModal" max-width="1000px">
          <v-card>
            <v-card-title class="headline">
              <v-btn @click="crearPDF()"><v-icon>mdi-printer</v-icon></v-btn> Reporte de venta
            </v-card-title>
            <v-card-text>
              <div id="factura">
                <header>
      ...
      >
        <template v-slot:[`item.opciones`]="{ item }">
          <v-icon small class="mr-2" @click="verventa(item)">mdi-pencil </v-icon>
          <v-icon small class="mr-2" @click="mostrarComprobante(item)"> mdi-printer</v-icon>
          <template v-if="item.estado">
      ...
      axios
        ...
        .then(function (response) {
          ...
          personaArray.map(function (x) {
            me.personas.push({ text: x.nombre, value: x.id }); //aqui nose si va sin el guion
          });
        })
